...
2. Create the following script. This script will be used to generate certificates for your Prism Central (can also be used to other servers)
   ...
   ```text
   cat << 'REALEND' > gencert.sh 
   #!/bin/bash
   
   if [ "$#" -ne 1 ]
   then
      echo "Usage: supply a server fqdn"
      exit 1
   fi

   # Assign the first argument/input for the command to the variable
   ...
   # Create a .ext file with SAN names for your DOMAIN
   ...
   openssl x509 -req -in $DOMAIN.csr -CA rootCA.pem -CAkey rootCA.key \
...
8. ``cat`` out the contents of ``rootCA.pem``, ``pc.ntnxlab.local.key`` and ``pc.ntnxlab.local.crt`` and copy them to the UserXX-WindowsToolsPC in separate files
...
7. Copy the ouput of previous cat command of ``rootCA.pem``, ``pc.ntnxlab.local.key`` and ``pc.ntnxlab.local.crt`` files into to your UserXX-WindowsToolsPC VM in any directory
...
2. Make sure the ccoctl file is present 
   ...
   ```
   ```bash title="Output"
   -rwxr----- 1 nutanix nutanix Apr 19 00:41 ccoctl
   ```
...
6. Check the ``manifests/openshift-machine-api-nutanix-credentials-credentials.yaml`` file to make sure the contents are good using ``cat``
